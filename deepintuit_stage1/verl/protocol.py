...
class DataProto:
    ...
    def print_size(self, prefix=""):
        ...
        size_of_numpy_array = get_size_recursively(self.non_tensor_batch)
...
def get_size_recursively(obj, seen_ids=None):
    """
    递归地计算一个 Python 对象的内存大小（字节）。
    能够处理嵌套的列表、元组、字典、NumPy数组和PyTorch张量。
    使用 seen_ids 来避免重复计算共享对象。
    参数:
        obj: 需要计算大小的对象。
        seen_ids: 一个集合，用于存储已经计算过的对象的id，以防止无限递归和重复计算。
    返回:
        对象的总大小（字节）。
    """
    # 初始化seen_ids集合，用于在递归调用中跟踪已访问的对象
    if seen_ids is None:
        seen_ids = set()
    # 获取对象的唯一ID
    obj_id = id(obj)
    # 如果该对象已经被计算过，则返回0，避免重复计算
    ...
    if isinstance(obj, dict):
        # 递归计算所有键和值的大小
        size += sum(get_size_recursively(k, seen_ids) + get_size_recursively(v, seen_ids) for k, v in obj.items())
    # 2. 如果是列表 (list) 或元组 (tuple)
    elif hasattr(obj, '__iter__') and not isinstance(obj, (str, bytes, np.ndarray, torch.Tensor)):
        # 递归计算容器中所有元素的大小
        size += sum(get_size_recursively(item, seen_ids) for item in obj)
    # 3. 如果是 NumPy 数组 (numpy.ndarray)
    elif isinstance(obj, np.ndarray):
        # .nbytes 属性提供了存储数据所需的确切字节数
        size += obj.nbytes
        # 如果数组的dtype是'object'，我们还需要递归计算它引用的每个对象
        if obj.dtype == 'object':
            size += sum(get_size_recursively(item, seen_ids) for item in obj)
    # 4. 如果是 PyTorch 张量 (torch.Tensor)
    elif isinstance(obj, torch.Tensor):
        # 对于Tensor，最准确的方法是 元素数量 * 每个元素的大小
        # sys.getsizeof() 已经包含了Tensor对象外壳的大小，我们只需加上数据存储的大小
        # 注意：这里我们不直接加，因为 getsizeof 已经包含了指针，而我们需要的是数据本身的大小。
        # 一个更稳妥的策略是直接返回 Tensor 的总存储大小。
        # Tensor 存储 = 对象外壳 + 数据。
        # 我们重新计算以获得更清晰的结果。
        size = sys.getsizeof(obj) + (obj.nelement() * obj.element_size())
    return size