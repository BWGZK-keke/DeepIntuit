...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        self.stop_token_ids = config.get("stop_token_ids", None)
        kwargs = dict(
            n=1,
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        position_ids = prompts.batch["position_ids"] if "position_ids" in prompts.batch else None
        ...
        if "multi_modal_data" in non_tensor_batch:
            # print(non_tensor_batch["multi_modal_data"][0]["video"][0].shape)
            vllm_inputs = []
        ...
        for input_data in vllm_inputs:
            ...
            input_data["pr.ompt_token_ids"] = list(input_data["prompt_token_ids"])
        ...
        with self.update_sampling_params(**kwargs):
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    # adhoc fix for <vid_pad> token
                    if response_ids[-1] in self.stop_token_ids:
                        response_ids[-1] = eos_token_id
                    response.append(response_ids)
                    if self.config.calculate_log_probs:
                        curr_log_prob = []
                        for i, logprob in enumerate(output.outputs[sample_id].logprobs):
                            if response_ids[i] in logprob:
                                curr_log_prob.append(logprob[response_ids[i]].logprob)
                            else:
                                for token_id in self.stop_token_ids:
                                    if token_id in logprob:
                                        curr_log_prob.append(logprob[token_id].logprob)
                                        break
            ...
            #     for sample_id in range(len(output.outputs)):
            #         # 1. Get the raw IDs
            #         raw_response_ids = output.outputs[sample_id].token_ids
                    
            #         # 2. FLATTEN: Ensure every element is a single integer
            #         sanitized_response_ids = []
            #         for tid in raw_response_ids:
            #             if isinstance(tid, list):
            #                 # Use the first token of the multi-token sequence
            #                 sanitized_response_ids.append(int(tid[0]))
            #                 print("#######################")
            #                 print("tid:", tid)
            #                 print("########################")
            #             else:
            #                 sanitized_response_ids.append(int(tid))

            #         # 3. Apply your stop token fix on the sanitized list
            #         if sanitized_response_ids[-1] in self.stop_token_ids:
            #             sanitized_response_ids[-1] = eos_token_id
            #             print("#######################")
            #             print("eos_token_id:", eos_token_id)
            #             print("########################")
            #             print("#######################")
            #             print("stop_token_ids:", self.stop_token_ids)
            #             print("########################")
                    # response.append(sanitized_response_ids)

                    # # 4. Handle Log Probs
                    # if self.config.calculate_log_probs:
                    #     curr_log_prob = []
                    #     # Use the raw logprobs from vLLM
                    #     raw_logprobs_list = output.outputs[sample_id].logprobs
                        
                    #     for i, logprob_dict in enumerate(raw_logprobs_list):
                    #         # Use the sanitized ID to look up the logprob
                    #         target_token = sanitized_response_ids[i]

                    #         if target_token in logprob_dict:
                    #             curr_log_prob.append(float(logprob_dict[target_token].logprob))
                    #         else:
                    #             # Fallback to stop tokens if the token isn't in top logprobs
                    #             found_fallback = False
                    #             for stop_id in self.stop_token_ids:
                    #                 if stop_id in logprob_dict:
                    #                     curr_log_prob.append(float(logprob_dict[stop_id].logprob))
                    #                     found_fallback = True
                    #                     break
                    #             if not found_fallback:
                    #                 # Absolute fallback to avoid length mismatch
