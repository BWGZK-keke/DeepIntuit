...
class NovaRayPPOTrainer:
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores):
        ...
        # samples = list(zip(outputs, scores, strict=True))
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    for i in range(len(batch.batch['input_ids'])):
                        ...
                        num_token = (input_ids == 13).sum().item()
                        # print("####################")
                        # print(batch.non_tensor_batch.keys())
                        # print("####################")
                        # if num_token * 4 != batch.non_tensor_batch['multi_modal_inputs'][i]['pixel_values_videos'].shape[0]:
                        #     print(self.tokenizer.decode(responses[responses!=0]))
                    # compute response mask
                    # NOTE: we assume that the response_mask is already computed in the reward_fn
